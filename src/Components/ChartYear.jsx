...
const SelectYear = ({ onChange, ...props }) => {
  ...
  return (
    <select onChange={(e) => onChange(e, e.target.value)} {...props}>
      {years.map((year) => (
...
function ApexChart() {
  ...
  useEffect(() => {
    ...
    // q(collection(db, 'transactions), where('timestamp', '>', start), where('timestamp', '<', end));
    // setTransactions(getRandomDates(start, end));
